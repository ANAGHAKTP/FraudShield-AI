...
def predict(data: Transaction):
    ...
    # ⚡ Bolt Optimization: Use predict_proba to derive predictions
    ...
    try:
        ...
        if probs.shape[1] > 1:
            ...
            # Fallback for dummy model trained on single class
            probability = 0.0
            prediction = model.classes_[0]
    except AttributeError:
        # Fallback if classes_ is not available
...
def predict_batch(data: BatchTransaction):
    ...
    # ⚡ Bolt Optimization: Calculate predict_proba once
    try:
        probs = model.predict_proba(df_features)
        if probs.shape[1] > 1:
            ...
            predictions = np.full(len(df_features), model.classes_[0])
    except AttributeError:
