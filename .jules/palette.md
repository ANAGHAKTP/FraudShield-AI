...
## 2024-08-08 - Added ARIA Roles to Dynamic Error Messages and Alerts
**Learning:** Found that dynamic form validation errors and alert panels lacked proper ARIA roles, meaning screen readers would not automatically announce them when they appeared on the screen.
**Action:** Always add `role="alert"` (and optionally `aria-live="assertive"`) to dynamically injected error messages and critical alert panels to ensure proactive announcement by assistive technologies. Additionally, ensure decorative icons within these panels use `aria-hidden="true"`.
