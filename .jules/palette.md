...
## 2024-07-01 - Dynamic Form Error Messages & Loading Buttons
**Learning:** Dynamic form validation or API error messages injected into the DOM must use `role="alert"` or `aria-live` regions to ensure they are proactively announced by screen readers. When replacing button text with a loading spinner, the accessible name is lost.
**Action:** Add `role="alert"` and `aria-live="assertive"` to dynamic error message containers. Maintain descriptive text alongside decorative loading spinners on buttons, and use `aria-busy="true"` to indicate processing state.
