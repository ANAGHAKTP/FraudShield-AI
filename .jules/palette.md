...
## 2024-07-01 - Accessible Dynamic Alerts
**Learning:** Dynamically rendered critical UI alerts (like the high-risk transaction notification panel) are not announced by screen readers when they mount if they lack proper ARIA live region attributes.
**Action:** Always include `role="alert"` and `aria-live="assertive"` on the container element of dynamically rendered critical UI alerts to ensure screen readers announce them immediately upon mounting.
