...
## 2024-07-01 - Accessible Loading Buttons
**Learning:** When implementing loading states on submit buttons, replacing the descriptive text entirely with a spinner removes the accessible name, causing screen readers to announce an unlabelled button while loading.
**Action:** Always maintain descriptive text alongside the spinner during loading states, add `aria-busy="true"` to communicate the state to assistive technologies, and use `aria-hidden="true"` on the spinner itself.
