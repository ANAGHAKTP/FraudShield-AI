...
## 2024-06-30 - Dynamic Alerts & Screen Readers
**Learning:** Found that dynamically rendered critical UI alerts (like high-risk transaction notifications) fail to alert screen-reader users immediately when mounted if missing proper roles, and decorative icons within them create noise.
**Action:** Always include `role="alert"` and `aria-live="assertive"` on dynamically mounted alert containers, and explicitly add `aria-hidden="true"` to purely decorative icons (like SVG components) within them.
