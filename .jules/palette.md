...
## 2026-06-27 - Retaining Accessible Names in Loading States
**Learning:** Replacing button text entirely with a loading spinner during async operations removes the accessible name, breaking screen reader context.
**Action:** When adding loading states to submit buttons, do not replace the descriptive text entirely with a spinner. Maintain the text alongside a decorative icon (with `aria-hidden="true"`) and add `aria-busy="true"` to reliably communicate the state to screen readers.
