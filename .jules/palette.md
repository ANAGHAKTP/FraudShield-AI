...
## 2024-07-02 - Accessible Loading States & Error Announcements
**Learning:** Found that replacing a submit button's text entirely with a loading spinner removes its accessible name, causing confusion for screen reader users. Additionally, dynamically rendered error messages in forms weren't being announced.
**Action:** Always maintain descriptive text alongside loading spinners in submit buttons (and add `aria-busy="true"`). For dynamic form errors, use `role="alert"` or `aria-live` to ensure they are proactively announced by assistive technologies.
