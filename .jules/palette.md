...
## 2024-08-14 - Accessible Loading States for Submit Buttons
**Learning:** Replacing a button's descriptive text entirely with a spinner during loading states removes its accessible name, causing screen readers to lose context.
**Action:** When adding loading states to submit buttons, always maintain the text alongside the spinner, add `aria-busy="true"` to the button, and ensure proper visual spacing (e.g., `margin-right: 0.5rem`) and text color matching.
