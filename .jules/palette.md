...
## 2026-07-09 - Dynamic Content Announcements
**Learning:** Found that dynamically injected DOM elements, such as form validation errors or real-time priority alerts, are not proactively announced by screen readers if they only conditionally render.
**Action:** Always add `role="alert"` or `aria-live="assertive"` to containers that dynamically inject critical text (like error states or system alerts) to ensure immediate screen reader announcements.
