...
## 2024-07-11 - Accessible Loading States for Buttons
**Learning:** Found that completely replacing a submit button's descriptive text with a loading spinner icon removes its accessible name, confusing screen readers during asynchronous operations.
**Action:** When adding loading states to submit buttons, always retain the descriptive text alongside a decorative icon (with `aria-hidden="true"`) and set `aria-busy="true"` on the button to reliably communicate the loading state to screen readers without losing context.
