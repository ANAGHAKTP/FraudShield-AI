...
## 2024-07-02 - Accessible Critical Alerts
**Learning:** Dynamically rendered critical UI alerts (like high-risk notifications) are not automatically announced by screen readers when they mount, making them invisible to visually impaired users despite their critical nature.
**Action:** Always include `role="alert"` and `aria-live="assertive"` on the container element of critical alerts to ensure screen readers properly announce them upon mounting, and add `aria-hidden="true"` to decorative icons inside them.
