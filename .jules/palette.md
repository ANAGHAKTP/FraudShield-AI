...
**Learning:** Found that replacing a submit button's entire text content with a loading spinner removes its accessible name, leaving screen readers with an empty button during async operations.
**Action:** Always maintain the descriptive button text alongside the spinner, add `aria-busy="true"` to communicate the loading state to assistive technologies, and use `aria-hidden="true"` for the spinner itself.
