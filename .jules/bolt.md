...
## 2024-08-20 - [Concurrent Independent DB Queries]
**Learning:** In Node.js backend services (like `gateway-nestjs`), executing independent database queries (e.g., Supabase select counts) sequentially introduces unnecessary network roundtrip latency.
**Action:** Always use `Promise.all()` to execute independent database queries concurrently rather than sequentially to minimize overall latency.
