...
## 2024-06-15 - [O(1) Bulk Updates for Transactions]
**Learning:** Using `Promise.all()` to resolve an array of sequential database `update` queries mapped over an array prevents blocking execution, but it still generates an O(N) volume of discrete database operations and network round trips. This severely throttles performance as batch sizes grow.
**Action:** Always prefer grouping independent bulk updates into a single `.upsert()` or `.in()` query whenever supported by the database adapter (like Supabase) to guarantee an O(1) performance profile.
