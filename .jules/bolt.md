...
## 2024-06-30 - [Concurrent Database Queries]
**Learning:** Sequential `await` calls for independent database queries (like Supabase select counts for total and fraud transactions) unnecessarily increase response latency by adding network roundtrips.
**Action:** Use `Promise.all()` to execute independent database queries concurrently, which minimizes network roundtrip latency.
