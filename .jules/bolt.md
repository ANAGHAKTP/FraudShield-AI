...
## 2024-08-17 - [Concurrent Independent DB Queries]
**Learning:** In Node.js backend services (like `gateway-nestjs`), sequential `await` calls for independent database queries (e.g., Supabase select counts for total and fraud transactions) unnecessarily increase endpoint latency by blocking on sequential network roundtrips.
**Action:** Always use `Promise.all()` to execute independent database queries concurrently rather than sequentially to minimize total network latency.
