...
## 2026-06-14 - [Concurrent DB Counts]
**Learning:** In the gateway-nestjs backend, the `/analytics/fraud-rate` endpoint fetched two independent counts sequentially via Supabase, unnecessarily inflating network round-trip time.
**Action:** Execute independent, read-only aggregate queries concurrently using `Promise.all()` to minimize response latency.
