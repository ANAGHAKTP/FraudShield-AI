...
## 2024-08-19 - [Concurrent Independent DB Queries]
**Learning:** In the gateway-nestjs backend, making independent database queries (like Supabase select counts) sequentially introduces unnecessary network roundtrip latency.
**Action:** Always use `Promise.all()` to execute independent database queries concurrently, which minimizes latency by processing the queries in parallel.
