...
## 2024-07-01 - [Concurrent DB Reads]
**Learning:** In the gateway-nestjs backend, executing independent Supabase database queries (like select counts) sequentially causes unnecessary network roundtrip latency.
**Action:** Always use `Promise.all()` to execute independent database queries concurrently rather than sequentially to minimize network latency.
