...
## 2024-07-25 - [Concurrent Database Queries]
**Learning:** The `analytics.service.ts` in gateway-nestjs performed independent database queries (fetching total transactions and fraud transactions) sequentially using `await`. This doubled the network roundtrip latency for the dashboard's analytics endpoints.
**Action:** Always use `Promise.all()` to execute independent Supabase queries concurrently when computing aggregate metrics.
