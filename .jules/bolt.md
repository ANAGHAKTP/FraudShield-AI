...
## 2024-07-25 - [Concurrent DB Queries for Aggregations]
**Learning:** In the gateway-nestjs backend, multiple independent aggregation queries (like total transaction count and fraud transaction count) were being executed sequentially, causing unnecessary cumulative network roundtrip latency.
**Action:** Always use `Promise.all()` to execute independent database queries concurrently, reducing overall latency to the maximum of the individual query times rather than their sum.
