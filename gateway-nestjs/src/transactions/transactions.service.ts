...
export class TransactionsService {
  ...
  async processTransaction(userId: number, transactionDto: any) {
    ...
    if (txError) {
      ...
      throw new InternalServerErrorException(
        'Database error handling transaction',
      );
    }
    ...
    try {
      predictionResult =
        await this.predictionService.predictFraud(dummyFeatures);
    } catch (error) {
      this.logger.error(
        'Failed to communicate with FastAPI Inference API',
        error,
      );
      throw new InternalServerErrorException(
    ...
    const finalStatus =
      predictionResult.fraud_probability > 0.8
        ? 'DECLINED'
        : predictionResult.fraud_probability > 0.4
          ? 'REVIEW'
          : 'APPROVED';
  ...
  async processBatchTransactions(userId: number, transactions: any[]) {
    ...
    if (txError) {
      ...
      throw new InternalServerErrorException(
        'Database error handling batch transactions',
      );
    }
    ...
    try {
      predictionResults =
        await this.predictionService.predictBatch(batchFeatures);
    } catch (error) {
      this.logger.error(
        'Failed to communicate with FastAPI Inference API (Batch)',
        error,
      );
      throw new InternalServerErrorException(
        'ML Prediction service unavailable',
    ...
    // 5. Update transaction statuses based on risk in grouped bulk operations
    // Optimization: Replaced O(N) concurrent single-row updates (Promise.all)
    // with O(1) grouped bulk updates using the .in() operator to minimize DB load.
    ...
    return {
      ...
      transactions: txData.map((t, idx) => ({
        ...t,
        prediction: predictionResults[idx],
      })),
    };
  }
  ...
  async getHighRiskTransactions(userId: number) {
    ...
    if (error) {
      throw new InternalServerErrorException(
        'Failed to fetch high-risk transactions',
      );
    }
    return data;
  }
