...
export class AnalyticsService {
  ...
  async getFraudRate(userId: number) {
    ...
    // ⚡ Bolt Optimization: Use Promise.all to fetch total and fraud counts concurrently
    // rather than sequentially to minimize network roundtrip latency.
    const totalCountPromise = supabase
      .from('Transactions')
      .select('*', { count: 'exact', head: true })
      .eq('user_id', userId);

    const fraudCountPromise = supabase
      .from('Transactions')
    ...
    ] = await Promise.all([totalCountPromise, fraudCountPromise]);
