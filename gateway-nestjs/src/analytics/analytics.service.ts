...
export class AnalyticsService {
  ...
  async getFraudRate(userId: number) {
    ...
    // ⚡ Bolt Optimization: Run independent Supabase queries concurrently using Promise.all
    // to eliminate sequential network waiting times.

    // 1. Fetch total transactions for user
    const totalTransactionsPromise = supabase
      .from('Transactions')
    ...
    const fraudTransactionsPromise = supabase
      .from('Transactions')
    ...
    ] = await Promise.all([totalTransactionsPromise, fraudTransactionsPromise]);
    ...
    if (fraudError)
      throw new InternalServerErrorException(
        'Error fetching fraud predictions',
      );
  ...
  async getHighRiskTransactions(userId: number, limit: number = 5) {
    ...
    const { data, error } = await supabase
      ...
      .eq('user_id', userId)
      .order('fraud_probability', {
        foreignTable: 'Predictions',
        ascending: false,
      })
      .limit(limit);
    ...
    if (error)
      throw new InternalServerErrorException(
        'Error fetching high risk transactions',
      );
  ...
  async getTrend(userId: number) {
    ...
    if (error)
      throw new InternalServerErrorException('Error fetching trend data');
    ...
    data.forEach((tx) => {
      ...
      const prediction = Array.isArray(tx.Predictions)
        ? tx.Predictions[0]
        : tx.Predictions;
