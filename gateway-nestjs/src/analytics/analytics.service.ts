...
export class AnalyticsService {
  ...
  async getFraudRate(userId: number) {
    ...
    // 1 & 2. Fetch total transactions and fraud predictions mapped to user transactions concurrently
    // Optimization: Executing independent DB queries concurrently via Promise.all reduces roundtrip time
    const [
